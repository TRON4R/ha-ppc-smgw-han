...
- **One meter, two sets of time windows**: the same meter can be added more than once if you want to evaluate it against *different* time windows — for instance once against your supplier's tariff windows and once against your grid operator's windows (section 14a EnWG, module 3). Keeping them in two devices is deliberate: each bill can be checked against exactly the windows it was billed by, and the two sets change independently — grid-fee windows usually at the turn of the year, supplier tariffs often mid-year. Each device therefore has its own settings, its own schedule and its own fetch time. Adding an already configured meter asks whether you really want a second evaluation and lets you name the device right there. Both entries read the same gateway and report the same total consumption; only the split across tariff zones differs, so each bill can be checked against its own set of windows. **Important:** add only *one* of the devices to the Energy dashboard, otherwise the same consumption is counted twice.
